...
const ansi_1 = __nccwpck_require__(938);
 ...
 */
const createLogger = (methods, config = {}, outputFunctions = console.log) => {
    const functions = Array.isArray(outputFunctions)
        ? outputFunctions
        : [outputFunctions];
    ...
    const completeConfig = Object.assign({ divider: ' ', newLine: '├-', newLineEnd: '└-', padding: 'PREPEND', paddingChar: ' ', color: true, exclude: [], filter: undefined, preProcessors: [], postProcessors: [] }, config);
    ...
    const completeMethods = Object.assign({}, ...Object.keys(finalMethods).map((a) => {
        if (typeof finalMethods[a] == 'string') {
            ...
            return {
                [a]: Object.assign(Object.assign({}, inferredMethodConfig), { label: finalMethods[a] }),
            };
        }
    ...
    const maxLength = Math.max(...Object.values(completeMethods).map((a) => typeof a.label === 'string'
        ...
        : a.label.length));
    const logger = {};
    return Object.assign(logger, ...Object.keys(completeMethods).map((methodHandle) => {
        ...
        return {
            [methodHandle]: (...s) => {
                ...
                for (const processor of completeConfig.preProcessors) {
                    inputs = processor(inputs, Object.assign({ name: methodHandle }, method), completeConfig);
                }
                // Generate the value we should output
                const lines = inputs
                    .map((value) => {
                    if (typeof value !== 'string') {
                        value = (0, node_util_1.inspect)(value, false, 3, completeConfig.color);
                    }
                    return value;
                })
                    .join('\n')
                ...
                for (const processor of completeConfig.postProcessors) {
                    parsedLines = processor(parsedLines, Object.assign({ name: methodHandle }, method), completeConfig);
                }
...
const ansiStyles = __nccwpck_require__(147);
const {stdout: stdoutColor, stderr: stderrColor} = __nccwpck_require__(145);
const {
	stringReplaceAll,
...
const chalkTag = (chalk, ...strings) => {
	...
	if (template === undefined) {
		template = __nccwpck_require__(932);
	}
...
const cssKeywords = __nccwpck_require__(858);
...
const conversions = __nccwpck_require__(949);
const route = __nccwpck_require__(202);
...
const hasFlag = __nccwpck_require__(17);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
var logger_1 = __nccwpck_require__(859);
var chalk_1 = __importDefault(__nccwpck_require__(44));
var core_1 = __importDefault(__nccwpck_require__(953));
...
var version = (__nccwpck_require__(598)/* .version */ .i8);
